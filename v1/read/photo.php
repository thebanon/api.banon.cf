...
if($_GET[3]) {
    ...
    $data['sql'][0][0] = $sql = "SELECT brands.name as brandname, photo.* FROM photo LEFT JOIN brands ON photo.app=brands.uid WHERE photo.uid LIKE '{$_GET[3]}'";
    ...
    if(mysqli_num_rows($query)>0) { $key = 0;
        ...
        $data['sql'][1] = $sql = "SELECT brands.name as brandname, photo.* FROM photo LEFT JOIN brands ON photo.app=brands.uid WHERE photo.category $category ORDER BY RAND() LIMIT 16";
        $query = mysqli_query($con, $sql);
        if(mysqli_num_rows($query)>0) { $key = 0;
            while($row = mysqli_fetch_assoc($query)) { 
        ...
        $data['sql'][2] = $sql = "SELECT comments.*, users.avi, users.username, users.uid as usr FROM comments LEFT JOIN users ON comments.usr=users.uid WHERE tbl LIKE 'photo' AND ref LIKE '{$_GET[3]}'";
        $query = mysqli_query($con, $sql);
        if(mysqli_num_rows($query)>0) { $key = 0;
            while($row = mysqli_fetch_assoc($query)) { 
        ...
        $data['sql'][3] = $sql = "SELECT COUNT(*) as count FROM activity WHERE ref LIKE '{$_GET[3]}' AND type LIKE 'view' AND tbl LIKE 'photo'";
        $data['count']['views'] = (int)mysqli_fetch_assoc(mysqli_query($con, $sql))['count'];

        $data['sql'][4] = $sql = "SELECT COUNT(*) as count FROM activity WHERE ref LIKE '{$_GET[3]}' AND type LIKE 'like' AND tbl LIKE 'photo'";
        $data['count']['likes'] = (int)mysqli_fetch_assoc(mysqli_query($con, $sql))['count'];

        $data['sql'][5] = $sql = "SELECT COUNT(*) as count FROM activity WHERE ref LIKE '{$_GET[3]}' AND type LIKE 'dislike' AND tbl LIKE 'photo'";
        $data['count']['dislikes'] = (int)mysqli_fetch_assoc(mysqli_query($con, $sql))['count'];

        $data['sql'][6] = $sql = "SELECT COUNT(*) as count FROM activity WHERE ref LIKE '$app' AND type LIKE 'subscribe' AND tbl LIKE 'photo'";
